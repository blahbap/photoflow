var app = angular.module('flow',['ngAnimate', 'linearPartitionModule']);

app.controller('mainController', ['$scope', '$http', 'linearPartitionService', function($scope, $http, lp) {
...
app.directive('imageflow', ['$window','$document', '$http', function($window, $document, $http){
	  return {
	  restrict:'E',
	  template: 
	  			// '<div>' +
	  			// '<img ng-repeat="photo in photos"' +
	  			// 'class="photo repeated-item" width="{{photo.optWidth}}"'  +
	  			// 'height="{{photo.optHeight}}"' +
	  			// 'ng-src="{{photo.url}}"></div>' 
